...
def get_testcase_crazy(policy="GA3C_CADRL"):
    agents = [
        ...
        Agent(-1.2, 0., -1.2, 5., 0.8, 1.0, np.pi / 2, policy_dict["RVO"], UnicycleDynamics, [OtherAgentsStatesSensor],
              1),
        Agent(-1.2, 2.0, -1.2, -3, 0.8, 1.0, -np.pi / 2, policy_dict["RVO"], UnicycleDynamics,
              [OtherAgentsStatesSensor], 2),
    ]
    return agents
...
def get_testcase_two_agents(policies=['learning', 'GA3C_CADRL']):
    ...
    goal_y = 3
    agents = [
        Agent(-goal_x, -goal_y, goal_x, goal_y, 0.5, 1.0, 0.0, policy_dict[policies[0]], UnicycleDynamics,
              [OtherAgentsStatesSensor], 0),
        Agent(goal_x, goal_y, -goal_x, -goal_y, 0.5, 1.0, np.pi, policy_dict[policies[1]], UnicycleDynamics,
...
def get_testcase_random(num_agents=None, side_length=4, speed_bnds=[0.5, 2.0], radius_bnds=[0.2, 0.8],
                        policies='learning', policy_distr=None, agents_dynamics='unicycle',
                        agents_sensors=['other_agents_states'], policy_to_ensure=None, prev_agents=None):
...
def small_test_suite(num_agents, test_case_index, policies='learning', agents_dynamics='unicycle',
    ...
    agents = cadrl_test_case_to_agents(cadrl_test_case, policies=policies, agents_dynamics=agents_dynamics,
                                       agents_sensors=agents_sensors)
    return agents


def full_test_suite(num_agents, test_case_index, policies='learning', agents_dynamics='unicycle', agents_sensors=[],
                    vpref_constraint=False, radius_bounds=None, prev_agents=None):
    cadrl_test_case = \
    preset_testCases(num_agents, full_test_suite=True, vpref_constraint=vpref_constraint, radius_bounds=radius_bounds)[
        test_case_index]
    agents = cadrl_test_case_to_agents(cadrl_test_case, policies=policies, agents_dynamics=agents_dynamics,
                                       agents_sensors=agents_sensors, prev_agents=prev_agents)
    return agents
...
def full_test_suite_carrl(num_agents, test_case_index, seed=None, other_agent_policy_options=None):
    ...
    agents.append(cadrl_test_case_to_agents([cadrl_test_case[0, :]], policies='carrl', agents_dynamics=UnicycleDynamics,
...
def get_testcase_random_carrl():
    ...
    agents = []
    agents.append(cadrl_test_case_to_agents([test_case[0, :]], policies='carrl', agents_dynamics=UnicycleDynamics,
                                            agents_sensors=[OtherAgentsStatesSensor])[0])
    agents.append(cadrl_test_case_to_agents([test_case[1, :]], policies='rvo', agents_dynamics=UnicycleDynamics,
                                            agents_sensors=[OtherAgentsStatesSensor])[0])
    agents[1].id = 1
    return agents
...
def cadrl_test_case_to_agents(test_case, policies='GA3C_CADRL', policy_distr=None,
    ...
    for i, agent in enumerate(test_case):
        ...
        if prev_agents is not None and policy_str == prev_agents[i].policy.str:
            ...
            new_agent = Agent(px, py, gx, gy, radius, pref_speed, heading, policy_dict[policy_str],
                              dynamics_dict[dynamics_str], sensors, i)
...
def make_testcase_huge(num_test_cases=1, num_agents=100, side_length=25, speed_bnds=[0.5, 2.0], radius_bnds=[0.2, 0.8],
                       policies='GA3C_CADRL'):
    ...
    for test_case in range(num_test_cases):
        for i in range(num_agents):
            ...
            while min_dist_to_others < 2.:
                ...
                if i > 0:
                    min_dist_to_others = min(
                        [np.linalg.norm(np.array([px - tc[px_ind], py - tc[py_ind]])) - tc[radius_ind] - radius for tc
                         in test_cases[test_case, :i, :]])
                else:
                    min_dist_to_others = np.inf
            ...
            while min_dist_to_others < 2. or dist_from_start < 5.:
                ...
                if i > 0:
                    min_dist_to_others = min(
                        [np.linalg.norm(np.array([gx - tc[gx_ind], gy - tc[gy_ind]])) - tc[radius_ind] - radius for tc
                         in test_cases[test_case, :i, :]])
                else:
                    min_dist_to_others = np.inf
...
def get_testcase_huge():
    ...
    agents = cadrl_test_case_to_agents(cadrl_test_case, policies='GA3C_CADRL', agents_dynamics=UnicycleDynamics,
